...
parser.add_argument( '--file_name', type=str, default='linear-data-LwF.h')
...
for i_epochs in range(num_epochs):
    ...
    for input_idx in range(num_train_inputs):
        ...
        output_prev = Variable(net_copy(indata).unsqueeze(dim=0).data, requires_grad=False) # net_copy(indata).unsqueeze(dim=0)
        output_new = torch.log_softmax(output/T, dim=1)   # compute the log of softmax values
        labels_prev = torch.softmax(output_prev/T, dim=1)
        print('output_prev: ', output_prev)
        print('output_new: ', output_new)
        print('labels_prev: ', labels_prev)
        ...
        '''
        output_prev = Variable(net_copy(indata).unsqueeze(dim=0).data, requires_grad=False) # net_copy(indata).unsqueeze(dim=
        outputs_S = F.softmax(output/T,dim=1)
        outputs_T = F.softmax(output_prev/T,dim=1)
        # Cross entropy between output of the old task and output of the old model
        loss_LwF = outputs_T.mul(-1*torch.log(outputs_S))
        loss_LwF = loss_LwF.sum(1)
        loss_LwF = loss_LwF.mean()
        '''
        ...
        # print(output)
        # print(label)
        loss_ori = criterion(output, label)
        loss = loss_ori  + loss_LwF
...
for input_idx in range(num_val_inputs):
    ...
    indata = torch.from_numpy(np.squeeze(np.load(f"inputs/inputs_fc_{input_idx}.npy"))).to(torch.float32) # inputs_fc_val_{input_idx}.npy
    indata = torch.flatten(torch.transpose(torch.reshape(indata, (32,29)), 0, 1)) #torch.flatten(torch.reshape((32,29)).transpose(indata, 0, 1))
    for i in range(in_size):
        indata[i] = indata[i] * eps_in
    #indata.requires_grad = True
