...
static void load_weights_async(Layer tile, Layer body, Layer layer, TileIndex index) {
  dma_transfer_async((DmaTransferConf) {
    .ext = dory_get_tile_3d(layer.addr.weights,
                            ...
                            body.weights.output_channel, 1*128, body.weights.input_channel,
                            1*128, 1,
                            0, 0, 0,
                            0, 0, 0,
                            8),
    ...
    .length_1d_copy = (int) tile.weights.output_channel * 8 * 1*128 / 8,
    .hwc_to_chw = 0,
    .stride_2d = 128,
    .stride_1d = 1,
    .dir = 1
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.scale + 44*index.output_channel,
    .loc = tile.addr.scale,
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.bias + 44*index.output_channel,
    .loc = tile.addr.bias,
    .length_1d_copy = tile.weights.output_channel * 4,
    .dir = 1
  });
...
void __attribute__ ((noinline)) BNReluConvolution1(void *args) {
  //////////////////////////////////////////////////////////////////////////
  ...
  const Layer layer = {
    .addr = {
      ...
      .scale = layer_args->L2_weights + 4096,
      .bias = layer_args->L2_weights + 4224,
  ...
  DoubleBuffer db_input = {
    .addrs = { l1_buffer + 0, l1_buffer + 0 + 20900 },
    .index = 0
  };
  DoubleBuffer db_output = {
    .addrs = { l1_buffer + 41808, l1_buffer + 41808 + 20900 },
    .index = 0
  };
  DoubleBuffer db_weights = {
    .addrs = { l1_buffer + 83616, l1_buffer + 83616 + 1496 },
    .index = 0
  };
  ...
  void * im2col = l1_buffer + 86632;
  void * pwt_buffer = im2col + 16;
  ...
  for(int iter=0; iter < total_tiles; iter++) {
    Address addr = {
      ...
      .scale = double_buffer_get_addr(db_weights) + 1408,
      .bias = double_buffer_get_addr(db_weights) + 1408 + 44,
