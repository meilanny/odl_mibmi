...
void mem_init() {
  ...
  // printf("flash_conf_init start\n");
  pi_open_from_conf(&flash, &flash_conf);
  // printf("openFromConf start\n");
  if (pi_flash_open(&flash)) {
    // printf("ERROR: Cannot open flash! Exiting...\n");
    pmsis_exit(-1);
  }

  // printf("openFilesystemOnFlash start\n");

  /* Open filesystem on flash. */
  pi_readfs_conf_init(&fs_conf);
  // printf("pi_readfs_conf_init done\n");
  fs_conf.fs.flash = &flash;
  // printf("fs_conf.fs.flash defined\n");
  pi_open_from_conf(&fs, &fs_conf);
  // printf("pi_open_from_conf done\n");
  if (pi_fs_mount(&fs)) {
    // printf("ERROR: Cannot mount filesystem! Exiting...\n");
    pmsis_exit(-2);
  }

  // printf("openRam start\n");
