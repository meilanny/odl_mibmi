...
static void load_weights_async(Layer tile, Layer body, Layer layer, TileIndex index) {
  dma_transfer_async((DmaTransferConf) {
    .ext = dory_get_tile_3d(layer.addr.weights,
                            ...
                            body.weights.output_channel, 1*16, body.weights.input_channel,
                            1*16, 1,
                            0, 0, 0,
                            0, 0, 0,
                            2),
    ...
    .length_1d_copy = (int) tile.weights.output_channel * 2 * 1*16 / 8,
    .hwc_to_chw = 0,
    .stride_2d = 4,
    .stride_1d = 1,
    .dir = 1
...
void __attribute__ ((noinline)) BNReluConvolution3(void *args) {
  //////////////////////////////////////////////////////////////////////////
  ...
  const Layer layer = {
    .addr = {
      ...
      .scale = layer_args->L2_weights + 128,
      .bias = layer_args->L2_weights + 256,
  ...
  DoubleBuffer db_input = {
    .addrs = { l1_buffer + 0, l1_buffer + 0 + 1896 },
    .index = 0
  };
  DoubleBuffer db_output = {
    .addrs = { l1_buffer + 1904, l1_buffer + 1904 + 7584 },
    .index = 0
  };
  DoubleBuffer db_weights = {
    .addrs = { l1_buffer + 9496, l1_buffer + 9496 + 384 },
    .index = 0
  };
  ...
  void * im2col = l1_buffer + 9904;
  void * pwt_buffer = im2col + 64;
  ...
  for(int iter=0; iter < total_tiles; iter++) {
    Address addr = {
      ...
      .scale = double_buffer_get_addr(db_weights) + 128,
      .bias = double_buffer_get_addr(db_weights) + 128 + 128,
